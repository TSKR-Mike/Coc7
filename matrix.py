...
def loading_matrix(data_collecting_method, window, names,comments:str=''):
    ...
    if data_collecting_method:#from excel files
        ...
        arrange_start = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200), 'input starting arrange number',
                                                   [list_preview.draw], [list_preview.handle_event], 'OK',
                                                   'CANCEL', backgroundColor=(90, 90, 150),
                                                   promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
        try:
            arrange_start= int(arrange_start)
        except TypeError:
            if arrange_start is not None:
                message_window.error('bad inputs for int ' + str(arrange_start))
            return
        arrange_end = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200), 'input ending arrange number(included)',
                                                   [list_preview.draw], [list_preview.handle_event], 'OK',
                                                   'CANCEL', backgroundColor=(90, 90, 150),
                                                   promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
        try:
            arrange_end = int(arrange_end)
        except TypeError:
            if arrange_end is not None:
                message_window.error('bad inputs for int ' + str(arrange_end))
            return
        column_start = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200), 'input starting column number',
                                                       [list_preview.draw], [list_preview.handle_event], 'OK',
                                                       'CANCEL', backgroundColor=(90, 90, 150),
                                                       promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
        try:
            ...
            if column_start is not None:
                message_window.error('bad inputs for int ' + str(column_start))
            return
        column_end = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200), 'input ending column number(included)',
                                                       [list_preview.draw], [list_preview.handle_event], 'OK',
                                                       'CANCEL', backgroundColor=(90, 90, 150),
                                                       promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
        try:
            ...
            if column_end is not None:
                message_window.error('bad inputs for int ' + str(column_end))
            return
        ...
        num = pyghelpers.textAnswerDialog(window, (0, 0, 1004, 200), 'How many arranges do you want?',
                                          'OK',
                                          'CANCEL', backgroundColor=(90, 90, 150),
                                          promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
        try:
            num = int(num)
            if num <= 0:
                message_window.error('the height of negative or zero is meaningless')
                return
        except:
...
def load_matrix_3_choices(window, clock, names,debug=False):
    ...
    if len(loading_type_selector.clicked_choices) != 0:
        ...
        if choice <= 1:
            ...
            # choice can only be 0(manually)->False or 1(.xlsx)->True;
        else:
            try:
...
class MatrixUi(subWindow):
                ...
                for curr in self.all_buttons_line2.Buttons:
                    if curr.handleEvent(event):
                        ...
                        if INDEX == 0:
                            ...
                            del self.all_matrix_data[self.all_matrix_data.index(self.curr_matrix)]
    ...
    def remove_matrix(self, matrix_name:str):
        if matrix_name not in self.all_matrix_dict.keys():
            message_window.error('the matrix "'+matrix_name+'" does not exist')
        else:
            del self.all_matrix_dict[matrix_name]
            index = 0
            for i in self.all_matrix_data:
