...
from EventPyghelpers import textAnswerDialogEventProgressing, textYesNoDialogEventProgressing
...
def data_analyze(window, clock):
    ...
    analyse_choice = CheckBox(32, all_statistic_types, 32, window, clock, first_x=50, first_y=30, each_add_x=0,
                              each_add_y=15, background_color=(90, 90, 150), auto_adjust=True, buttons_adjust_length=40,
                              bkg_width_adj=400)
    ...
    for statistic_type in analyse_types:
        if statistic_type not in [8, 11, 12, 13, 14, 27, 28, 29, 31]:#just need single group of data as input.
            ...
            if statistic_type == 8:
                if data_collecting_method:
                    percent = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200), 'input the percent number, smaller or equal to 100 and bigger or equal to 0',
                                                                       [list_preview.draw], [list_preview.handle_event],
                                                                       'OK',
                                                                       'CANCEL', backgroundColor=(90, 90, 150),
                                                                       promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
                else:
                    percent = pyghelpers.textAnswerDialog(window, (0, 0, 1004, 200), 'input the percent number, smaller or equal to 100 and bigger or equal to 0',
                                                                   'OK',
                                                                   'CANCEL', backgroundColor=(90, 90, 150),
                                                                   promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
                try:
                    ...
                    message_window.error("Bad inputs for int:"+str(percent))
                    return
                results.append('percentile:'+str(percentile(data, percent)))
            # moving_average
            elif statistic_type == 13:
                if data_collecting_method:
                    window_size = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200),
                    ...
                    window_size = pyghelpers.textAnswerDialog(window, (0, 0, 1004, 200),
                                                                   "input the window size,bigger than 0(it will traverse and take the average of each window-size's data)",
                                                                   'OK','CANCEL', backgroundColor=(90, 90, 150),
                                                                   promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
                try:
                    ...
                    message_window.error("Bad inputs for int:" + str(window_size))
                    return
                results.append('moving-average:' + str(moving_average(data, window_size)))
            # EWMA
            elif statistic_type == 14:
                if data_collecting_method:
                    alpha = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200),
                    ...
                    alpha = pyghelpers.textAnswerDialog(window, (0, 0, 1004, 200),
                                                             "input the weight(The closer the data is to the current loc, the greater the weight;and vice versa)",
                                                             'OK','CANCEL', backgroundColor=(90, 90, 150),
                                                             promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
                try:
                    alpha = int(alpha)
                    if not 0 < alpha:
                        message_window.error("the percent value is wrong:" + str(alpha))
                        return
                except:
                    message_window.error("Bad inputs for int:" + str(alpha))
                    return
            ...
            if refresh_data:
                ...
                if data_collecting_method:  # from excel files
                    ...
                    if not choice:  ####by column ################################################################################
                        ...
                        line = textAnswerDialogEventProgressing(window, (0, 0, 1004, 200), 'input line number',
                                                                [list_preview.draw]
                                                                , [list_preview.handle_event], 'OK',
                                                                'CANCEL', backgroundColor=(90, 90, 150),
                                                                promptTextColor=(0, 0, 0), inputTextColor=(0, 0, 0))
